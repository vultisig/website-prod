...
const CHAINS: Chain[] = [
  { name: "MayaChain", dot: "#2cfffc", icon: "mayachain", x: 216, y: -19 },
  { name: "Bitcoin", dot: "#f7931a", icon: "bitcoin", x: 368, y: 19 },
  { name: "Hyperliquid", dot: "#97fce4", icon: "hyperliquid", x: 458, y: 120 },
  { name: "Ethereum", dot: "#8c8c8c", icon: "ethereum", x: 500, y: 259 },
  { name: "Dogecoin", dot: "#ba9f33", icon: "dogecoin", x: 464, y: 399 },
  { name: "BNB Chain", dot: "#f0b90b", icon: "bnb", x: 358, y: 501 },
  { name: "THORChain", dot: "#18e4cd", icon: "thorchain", x: 216, y: 539 },
  { name: "Solana", dot: "#aa51ea", icon: "solana", x: 93, y: 501 },
  { name: "TRON", dot: "#ff060a", icon: "tron", x: -9, y: 399 },
  { name: "XRP Ledger", dot: "#ffffff", icon: "xrp", x: -62, y: 259 },
  { name: "Zcash", dot: "#f3b724", icon: "zcash", x: -10, y: 119 },
  { name: "Polygon", dot: "#6600ff", icon: "polygon", x: 89, y: 19 },
]
 ...
 */
function ChainPill({ name, dot, icon, x, y }: Chain) {
  return (
    <li
      style={{ left: percent(x), top: percent(y) }}
      className={`group static flex h-[38px] items-center gap-2 whitespace-nowrap rounded-[10px] border border-v5-border-light bg-v5-surface-2 px-[17px] text-v5-body-s font-medium text-v5-text-primary backdrop-blur-[2px] transition-colors ${RUN} [@media(hover:hover)]:hover:border-[#263b5a] [@media(hover:hover)]:hover:bg-[#263b5a] md:absolute`}
    >
      <span
        aria-hidden
